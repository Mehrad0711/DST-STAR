...
def main(args):
    ...
    train_data_raw = processor.get_train_instances(args.data_dir, tokenizer, args.subsample)
    print("# train examples %d" % len(train_data_raw))
    
    dev_data_raw = processor.get_dev_instances(args.data_dir, tokenizer, args.subsample)
    print("# dev examples %d" % len(dev_data_raw))
    
    test_data_raw = processor.get_test_instances(args.data_dir, tokenizer, args.subsample)
    ...
    for epoch in trange(int(args.n_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader)):
            ...
            if epoch > args.n_epochs / 2 and step > 0 and step % args.eval_step == 0:
                ...
                logger.info(
                    "*** Step=%d, Dev Loss=%.6f, Dev Acc=%.6f, Dev Turn Acc=%.6f, Best Loss=%.6f, Best Acc=%.6f ***" % \
        ...
        if (epoch + 1) % args.eval_epoch == 0:
            ...
            logger.info(
                ...
                epoch, last_update, eval_res['loss'], eval_res['joint_acc'], eval_res['joint_turn_acc'], best_loss,
                best_acc))
